...
struct HostDashboardView: View {
        ...
        List {
            if let user = session.appUser {
                Section("Paiements") {
                    ...
                    Button("Rafraîchir profil Stripe") {
                        Task { await session.refreshUserProfile() }
                    }
                }

                Section("Repas") {
                    Button("Créer un repas") { viewModel.showingCreateMeal = true }
            ...
            Section("Commandes reçues") {
                if viewModel.receivedOrders.isEmpty {
                    ...
                    ForEach(viewModel.receivedOrders) { order in
                        // ✅ Bug corrigé : order.id est String (non-optionnel)
    ...
    private func stripeStatusRow(user: AppUser) -> some View {
        HStack {
            Text("Stripe status")
            Spacer()
            Text(user.isStripeReady ? "Onboardé" : "Non prêt")
...
// MARK: - HostOrderRow (privé à ce fichier)
...
private struct HostOrderRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                // ✅ order.id est String — plus de optional chaining inutile
